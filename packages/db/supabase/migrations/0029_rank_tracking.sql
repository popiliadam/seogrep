-- Migration 0029: the RANK-TRACKER storage — the keywords a tenant asked to watch, and the SERP
-- measurements taken for them. TWO tables, because the two answer two different questions and
-- neither can be derived from the other.
--
-- NUMBERING NOTE FOR THE MERGE (read this first). A sibling branch (feat/research-keyword-runs)
-- also carries a 0029 — `keyword_research_runs`. Both cannot keep the number: index.test.ts pins
-- the migration numbering as a GAP-FREE 1..N run against SCHEMA_VERSION, so whichever lands second
-- must be renamed to 0030 and its SCHEMA_VERSION bumped with it. This file was written on a branch
-- based on main@ca645a9, where 0028 is the highest committed migration; naming it 0030 here would
-- have left a hole at 29 and turned the fast gate red on this branch, which is why it is not
-- numbered 0030 today. The rename is mechanical and touches exactly two places: this file's NAME
-- and packages/db/src/index.ts's SCHEMA_VERSION. Nothing below refers to the number again, and no
-- spec asserts it.
--

...

{
  ...
  while (i <= n) {
    ...
    if (c == dq) {
      ...
      # the bare word, while every shape below matches bare words only (audit R7).
      ...
      # before R7. Folding it instead would let a quoted REVOKE on some other table cancel
      # a real GRANT. Since the span that passes is all-lowercase by construction, copying
      # it from $0 and from the folded line are byte-identical.
      e = dqend(line, i)

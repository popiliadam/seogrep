...
describe("backlink_changes credit path against the local stack", () => {
  ...
  it("(f) a stranger is refused with ZERO rows; the OWNER's own project resolves and is charged", async () => {
    ...
    const domain = `blchanges-${randomUUID().slice(0, 8)}.com`;
    const { data, error } = await service
      .from("projects")
    ...
    expect(mine.content[0]?.text).toContain(`Backlink history for your project "${domain}"`);
    expect(mine.content[0]?.text).toContain("1,334 backlinks");
    const ownerRows = await ledgerRows(owner.userId);
    expect(ownerRows.map((r) => r.kind)).toEqual(["purchase", "spend_reserve", "spend_commit"]);
    expect(ownerRows[1]?.delta).toBe(-TOOL_COSTS.backlink_changes);
    expect(ownerRows[1]?.tool).toBe("backlink_changes");
    expect(balanceOf(ownerRows)).toBe(300 - TOOL_COSTS.backlink_changes);
  });
});

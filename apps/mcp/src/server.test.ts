...
// both routes advertise the same 21 tools) and a REAL limiter (to prove one shared budget).
...
describe("mcp gateway fixed header-key endpoint", () => {
  it("POST /mcp with x-api-key serves the SAME tool surface as the path form (all 21 tools)", async () => {
    // Injecting the production tool set proves the fixed route reaches the real MCP
    // handler, not a reduced one: both routes must advertise the identical 21 tools
    // (19 + list_gsc_properties + track_gsc_property, 2026-08-13).
    ...
    try {
      ...
      expect(headerBody.result.tools).toHaveLength(21);

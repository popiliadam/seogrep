...
Each of those fields is **omitted when DataForSEO did not send it**, rather than padded with `n/a`; a row with nothing extra to say stays a single line. A dated line under the table says when the vendor last refreshed the CPC and competition figures, and says so in a sentence once that is over a month old.

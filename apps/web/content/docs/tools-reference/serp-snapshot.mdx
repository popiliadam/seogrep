...
- **Found** — with DataForSEO's own `rank_group` (its rank among organic results) and `rank_absolute` (its rank among every element on the page, including featured snippets and ad blocks). Both are reported, because they disagree whenever a SERP feature sits above the result, and that gap is itself the finding.

...
So those two modes carry a **warning** above their results, and a **default search-volume ceiling** that holds back the very high-volume national queries. The answer always names the ceiling it applied; pass `max_volume` to move it, or `max_volume: 0` to remove it and see the unfiltered set.

The ceiling is a **partial** remedy and is described as one: a volume bound cannot remove an off-subject keyword of ordinary volume, which is exactly what `ideas` returned. SeoGrep does not read meaning and will not filter rows by it.

...
- **Connected, but no property is mapped** → [`list_gsc_properties`](/docs/tools-reference/list-gsc-properties), then [`track_gsc_property`](/docs/tools-reference/track-gsc-property). Both are free. The Google account works, so this rung does **not** send you round another OAuth loop — what is missing is the mapping, and until it exists a Search Console pull cannot run.
...
- **Everything fresh, but nothing analysed yet** → [`find_quick_wins`](/docs/tools-reference/find-quick-wins) first: it reads the Search Console data you already pulled and names the pages closest to moving up. A report is worth generating once there are findings to put in it.

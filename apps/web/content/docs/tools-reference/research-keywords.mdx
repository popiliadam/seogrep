...
A keyword the provider holds **nothing** on comes back as _"no data returned for this keyword"_. It is never printed as `volume 0` — "nobody has a figure for this" and "nobody searches this" are different facts that lead to different decisions — and a genuine zero, when the provider does report one, is still printed as `volume 0`.

...
| `limit` | integer | No | How many filtered backlinks to examine (1-300, default 100). DataForSEO bills per returned row, so this is the price control, not a display preference. The candidate domains are derived from exactly these rows. |
| `network_limit` | integer | No | How many referring NETWORKS (IP subnets) to return (1-50, default 20). Billed per row on the same tariff. A separate axis: several linking domains sitting in one subnet is a fact about addresses, not a verdict. |
...
`disavow_candidates` needs a **paid credit balance**. It reads live data from a paid third-party provider — three requests per call, the most of any SeoGrep tool — so it is not available on trial credits. Buy any credit pack and it unlocks straight away; your existing credits are untouched and keep working for crawls, audits, reports and Search Console tools.

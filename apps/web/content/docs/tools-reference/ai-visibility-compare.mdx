...
| `confirm` | boolean | No | Set to true to re-run a call whose estimated cost exceeded the confirmation threshold. Optional, and only meaningful after this tool has answered with a confirmation prompt: nothing is charged until the call is re-run with it. |

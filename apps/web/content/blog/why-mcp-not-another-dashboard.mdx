...
"Your own OAuth" is worth pausing on. SeoGrep never sees a shared or pooled Search Console account — the GSC-backed tools only work because you connected your own property, and they only ever see your own data. A crawl feeds the audits, the audits feed the report, and the router tool can point at all of it — the agent moves between them by calling the next tool, not by you copying a URL from one screen into another.

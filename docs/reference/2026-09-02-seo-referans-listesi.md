...
| R-1.7 | PSI API v5 iki ayrı veri döndürür: **field = CrUX gerçek kullanıcı verisi**, **lab = Lighthouse**. Google, **CrUX verisini bu API'den kaldırmayı planladığını** ve ayrı CrUX API'ye geçilmesini önerdiğini belirtiyor | https://developers.google.com/speed/docs/insights/v5/get-started | 2026-09-02 | audit_speed |
| R-1.8 | PSI `runpagespeed` parametreleri: `url` (zorunlu), `category` (accessibility·best-practices·performance·seo), `strategy` (**varsayılan desktop**), `locale`. Lighthouse sürümü yanıttaki `lighthouseVersion` alanından okunur; dokümanda **sabit bir sürüm pinlenmemiştir** | https://developers.google.com/speed/docs/insights/v5/reference/pagespeedapi/runpagespeed | 2026-09-02 | audit_speed |
...
| R-3.13 | Canonical **JavaScript ile değil HTML'de** verilmelidir; SPA'larda soft-404 redirect ya da noindex ile çözülür | https://developers.google.com/search/docs/crawling-indexing/javascript/javascript-seo-basics | 2026-09-02 | audit_tech |
...
| audit_onpage | R-4.1–R-4.5, R-4.9, R-3.15, R-3.16, R-6.2 | **Karakter sınırı efsanesi:** Google title/description için sınır yayımlamıyor (R-4.2, R-4.4). `noarchive` bulgusu artık ölü (R-3.16) |
| audit_tech | R-3.1–R-3.16, R-3.19–R-3.24, R-4.10, R-4.11, R-9.4 | AI crawler token listesinin bayatlaması (Claude-SearchBot, OAI-SearchBot, GoogleOther-*) |
...
| audit_speed | R-1.1–R-1.8 | Eşiklerin hardcode edilmesi ve FID kalıntısı; PSI'dan CrUX'un kaldırılma planı (R-1.7) |
...
| ai_visibility_compare | R-5.5, R-5.7, R-5.8, R-8.5, R-8.7 | Query fan-out (R-5.5) yüzünden tek-kelime karşılaştırmasının yanıltıcı olması |

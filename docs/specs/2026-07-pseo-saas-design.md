...
**Servisler:** Supabase Postgres (Auth + **RLS her tabloda açık**) · pg-boss job kuyruğu (Postgres-içi; D27 — beta'da Redis yok, hacim artarsa BullMQ'ya geçiş yolu açık) · Paddle (MoR; onboarding reddi halinde fallback: Lemon Squeezy) · PostHog (funnel: signup→connect→ilk tool→satın alma) · Resend (transactional e-posta).

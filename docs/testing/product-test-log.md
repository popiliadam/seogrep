...
| 63 | Ş | `revoke.ts` / disconnect | **Tek projede "Disconnect", o Google hesabındaki BÜTÜN projelerin token'ını öldürüyor olabilir — ve hiçbir yerde yazmıyor.** `revoke.ts`'in kendi yorumu söylüyor: *"Revoking a refresh token invalidates the whole grant — every access token derived from it."* Google'ın revoke ucu token'ı değil **grant**'ı iptal eder; bugün her projenin token'ı aynı grant'tan gelir (aynı client + aynı Google hesabı + aynı scope). Dolayısıyla bir projeyi koparmak, aynı hesaba bağlı öteki projeleri de kimlik bilgisinden eder. **#52'nin şekline uyuyor** (4 ölü / 2 canlı, canlı olanlar en son onaylananlar) ama **KANITLANMADI — ölçülmedi, yalnız mekanizmadan türetildi.** Sınanabilir: iki proje bağla, birini kopar, ötekinde `pull_gsc_data` koş. Kod-seviyesinde kesin olan şey, disconnect'in yarıçapının kullanıcıya HİÇ anlatılmaması. Hesap-bazlı yeniden kurgu (spec `2026-08-10-gsc-account-oauth-design.md`) bu sınıfı yapısal olarak yok eder: bir hesap = bir grant olunca "Disconnect" dürüst bir şey ifade eder. | 🟡 | açık — spec'te ele alındı |
| 52 | Ş | `pull_gsc_data` · `connect_gsc` | **ALTI bağlı projenin DÖRDÜNDE Google refresh token'ı ÖLÜ; ürün "bağlı" diyor.** `pull_gsc_data` adstark · bigcattr · katrenur · dentnotion'da jenerik çökme cümlesi döndü; çalışan iki site (bayder, rkturizm) **dün yeniden onaylanan tam olarak o ikisi**. Sebep tahmin edilmedi, `flyctl logs -a seogrep-mcp` ile okundu: 12 referans kimliğinin 12'si de `Google token endpoint failed (400): invalid_grant`. Zarar zinciri: (1) `connect_gsc` *"already connected — property https://…"* diyerek bağlantıyı **sağlıklı ilan ediyor**; (2) `pull_gsc_data` *"failed unexpectedly, quote reference X"* diyor, yani kullanıcı bunun bir yetki sorunu olduğunu ve **yeniden onaylayarak düzeltebileceğini** öğrenemiyor; (3) alttaki üç tool tam ücretle bayat veri satıyor (**#53**). Bu, #51'in tarif ettiği boşluğun (property doğrulanıyor, KİMLİK BİLGİSİ hiç denenmiyor) **ölçülmüş yaygınlığı**: tek site anomalisi değil, **çoğunluk durum**. #51'in "yeniden bağlanma iyileştirir" tespiti de doğrulandı. Para tarafı DOĞRU: 12 çağrının 12'si de 0 kredi. | 🔴 | açık |
| 53 | Ş | `find_quick_wins` · `detect_cannibalization` · `analyze_content_decay` | **Token ölüyken üç GSC tool'u tam ücretle TARİHSİZ, bayat veri döndürüyor.** Token'ı ölü dört projede üçü de başarıyla koştu ve 10'ar kredi aldı; sundukları veri son başarılı `pull`'a ait (bigcattr'ın decay çıktısı 1. oturumunkiyle aynı: 2520 → 1143). **Ölçülen asimetri, 32 çıktıda istisnasız:** crawl tabanlı tool'lar verisini **14/14 tarihliyor** (*"(crawl from 2026-08-09T…)"*), GSC tabanlı tool'lar **18/18 tarihlemiyor**. Bağlantısı Ocak'ta ölen müşteri yıl boyunca Ocak verisi için öder ve bunu hiçbir çıktıdan anlayamaz. Crawl ailesi bu problemi zaten çözmüş; aynı disiplin GSC ailesine uygulanmamış — *"bu veriyi başka kim okuyor?"* sorusunun **beşinci** vakası. **KANITLANDI (2026-08-10, aynı gün, kendi kontrol grubuyla):** operatör yeniden onay yaptı; `pull_gsc_data` yalnız **adstark**'ta düzeldi, bigcattr/katrenur/dentnotion hâlâ `invalid_grant` (Fly log'u). Bir saat arayla aynı üç tool yeniden koşuldu ve çıktılar **hash'lendi**: adstark (taze veri) → `find_quick_wins` ve `analyze_content_decay` **DEĞİŞTİ**; token'ı ölü üç site → **9/9 çıktı BİREBİR AYNI**, üstelik her biri yine 30 kredi aldı (toplam 90 kredi, tek satır yeni bilgi yok). Yani bu artık mekanizmadan türetilmiş bir iddia değil, **doğrudan ölçülmüş bir olgu** — ve kontrol grubunu kurmak gerekmedi, ölçüm kendi kontrolünü üretti. | 🔴 | açık |
| 54 | Ş | `crawl_site` (ön-keşif) | **Satın alma öncesi site boyutu tahmini KIRPIK, ama tam sayıymış gibi sunuluyor — 6,6 kata kadar.** Mesaj *"~28 pages discovered; this crawl covers up to 100 of them (20 credits)"* diyor, müşteri "sitem 28 sayfa, hepsi taranacak" diye okuyor; aynı koşuda crawl **184** URL keşfediyor ve 26'sını tarıyor (%14). Ölçüm (ön-tahmin → gerçek keşif): dentnotion 28→184 · adstark 34→85 · rkturizm 90→156 · bayder 64→72 · katrenur 28→29 · seogrep 49→52. **Mekanizma KODDAN okundu:** `estimateSiteSize` toplam **8 sn**lik `PRE_DISCOVERY_BUDGET_MS` ile sınırlı ve bunu kök + tüm alt sitemap'lerle paylaşıyor; `loadSitemapSeeds` içinde `const childTimeout = hopTimeout(deadline, timeoutMs); if (childTimeout <= 0) break;` — bütçe biterse kalan çocuklar **hiç istenmiyor** ve fonksiyon erken durduğuna dair **sinyal vermeden** elindekini döndürüyor. Sapma sitemap sayısıyla birebir örtüşüyor (5 sitemap → 6,6× · 1 sitemap → ~1,0×). Kodun kendisi bu tuzağı **homepage yedek dalında görmüş** (*"truncation never overstates the site's size"*), birincil sitemap dalında görmemiş. | 🔴 | açık |
| 55 | Ş | `compare_competitors` | **Locale uyarısı bu tool'da HİÇ YOK — 0/7.** `ranked_keywords` 7 sitenin 4'ünde uyarıyor; `compare_competitors` yedisinin hiçbirinde uyarmıyor, `adstark.com.tr` ve `bayder.com.tr` gibi apaçık ccTLD'ler dahil. `localeHint()` `tools/ranked-keywords.ts` içinde yaşıyor ve öteki tool'a hiç uygulanmamış. #39'un n=7'de doğrulanması ve kapsam genişlemesi. | 🟡 | açık |
| 56 | Ş | `ranked_keywords` | **Locale uyarısının EŞİĞİ, tam da en çok gerektiği yerde susturuyor.** `THIN_RESULT_ROWS = 5` (`tools/ranked-keywords.ts:106`): sonuç ≥5 satırsa uyarı basılmaz. rkturizm.com aynı 65 kredi ile: varsayılan `en`/2840 → **11** keyword, en iyi #6, uyarı **YOK**; doğru `tr`/2792 → **482** keyword, en iyi **#1**, en yüksek hacim 880. **44 kat.** Site en yanlış temsil edildiği durumda "yeterince sonuç var" sayılıyor. bigcattr aynı (11 sonuç, Türk sitesi, `.com`, uyarı yok). Uyarının çalıştığı 4 vakanın hepsi zaten sonucu 0-3 olan, yani müşterinin sorunu **kendi göreceği** vakalar. H9/#18'in ölçülmüş gerekçesi; migration 0021'i doğrudan destekler. | 🟡 | açık |
| 57 | Ş | `compare_competitors` | **En pahalı tool, 7 sitenin 2'sinde 90 krediye ÜÇ SATIR döndürdü.** katrenur ve seogrep için çıktının TAMAMI: *"DataForSEO has no competitors on record for this domain … • katrenur.com (target) — No organic ranking data on record."* İki kusur birleşiyor: yanlış varsayılan locale → veri yok → 90 kredi → ve **#55 yüzünden locale'in sebep olabileceği söylenmiyor**, üstelik `ranked_keywords` dakikalar önce tam da bu alan adı için uyarmıştı. | 🟡 | açık |
| 58 | Ş | `compare_competitors` | **Otomatik rakip seçimi, sonuç dönen 5 sitenin 3'ünde DEV site verdi.** rkturizm → **facebook.com** (110.726.575 SERP) · bigcattr → **instagram.com** (85.932.260) · bayder → **tff.org** (Türkiye Futbol Federasyonu). Makul olan ikisi: adstark → brandaft.com (17 SERP), dentnotion → mehmetekerdisklinigi.com (2 SERP, **gerçek yerel rakip**). #17/#26 n=7'de: dev-site sorunu **çözülmedi ve çoğunluk sonuç**. #26'nın "kök vendor'da" tespiti duruyor; bu tur yalnız yaygınlığı ölçtü. | 🟡 | açık |
| 59 | Ş | `crawl_site` | **Ön-keşif başarısız olunca cümle SESSİZCE düşüyor.** Altı sitede mesaj *"~N pages discovered; this crawl covers up to 100 of them"* ile biter; `www.bigcattr.com`'da o cümle **hiç yok** — mesaj `job_id`'den sonra kesilir, iş 6,2 sn'de biter, **20 kredi yine alınır**, ardından audit üçlüsü **50 kredi daha** alır. Hiçbir çıktıda "siteni getiremedik" yazmaz. Yokluk bir hata mesajından çok daha sessizdir. #37'nin ürün-tarafı yüzü. | 🟡 | açık |
| 60 | Ş | `audit_tech` | **"Robots conflicts" kuralı KENDİ sitemizde 3/3 yanlış pozitif.** `seogrep.com`: `/login` (11 sayfadan linkli), `/signup` (10), `/forgot-password` (1) — üçü de **tasarım gereği** noindex ve **tasarım gereği** her sayfadan linkli. Kural kimlik-doğrulama/yardımcı sayfalar için muafiyet tanımıyor. **Kural değerli, muafiyeti eksik:** aynı kural bayder'de gerçek bir bulgu üretti (4xx olan `/bagimlilik-tedavisi`, 6 sayfadan linkli — Faz C bunu bağımsız olarak da yakaladı). | 🟡 | açık |
| 61 | Ş | `crawl_site` | **Tekrarlanabilirlik kötüleşti (#41 güncellemesi).** adstark, aynı parametreler, üç koşu: **37** (2026-08-09) · **47** (aynı gün) · **22** (2026-08-10). En yüksek/en düşük = **2,1 kat**. Kapsama tabanlı hiçbir kıyas tekrarlanabilir değil. | 🟢 | açık |
| 62 | Ş | `crawl_site` · `audit_tech` | **rkturizm ilk kez ZAMAN değil URL tavanına takıldı.** `audit_tech` 55 URL için *"max URL limit reached"* diyor, *"time budget exhausted"* değil; crawl tam **100**'de (varsayılan `max_urls`) durdu. Mesaj iki sebebi doğru ayırıyor — bu **iyi** ve korunmalı; #5'in "bağlayıcı sınır her zaman zamandır" iddiasının ilk karşı-örneği. | 🟢 | açık |

...
**Karar:** **DÜZELTME GEREKLİ** — tool teknik olarak doğru çalışıyor ve tek bir bayat SEO tavsiyesi üretmiyor, fakat (a) `audit_content_runs` basamağının anlamı yalnız Docker isteyen DB şeridinde korunuyor, (b) tavsiye kataloğu canlı 38 tool'un 22'sini hiç anmıyor — AI görünürlük ailesi dahil.
...
Ancak `card-map.ts:64` → `export const CARDED_TOOLS: ReadonlySet<ToolName> = new Set<ToolName>(["get_credit_balance"]);` — yani `whats_next` PLANLI, henüz SEVK EDİLMEMİŞ.

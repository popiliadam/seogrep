...
**Karar (ölçüm turu, 2026-09-02):** DÜZELTME GEREKLİ — canlı davranış ailenin en olgunu (7 negatif dalın 7'si doğru ve hiçbiri yazmadı), ama iki açık: upsert'in kiracı çakışma hedefi kapısız, ve iş emrinin istediği "eşleşmeyen property" negatifi bu tool'un API'sinde İFADE EDİLEMİYOR.

**Karar (kapanış, 2026-09-02):** KAPANDI (dilim 1 düzeltmesi, #198 + #203 + #206 [CI'da, merge bekliyor]) — iki P1 de kapandı: `onConflict` kiracı hedefi pinlendi (TGP-1), sweep kapsaması #198'de kapanmıştı (TGP-4). **Kalan:** TGP-2 ve TGP-3 (P2, ikisi de açık); TGP-5 kapandı ama **canlı doğrulaması yok**.

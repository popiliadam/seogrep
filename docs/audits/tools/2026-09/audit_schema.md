...
| S-B8 | P2 | **Kart planlı, sevk edilmemiş; yapılı kanal boş — oysa `SchemaReport` zaten jsonb olarak saklanıyor.** Canlı `tools/list` `_meta` taşımıyor, `tools/call` yalnız `content` taşıyor | `card-map.ts:31` + `:62`; canlı envelope ölçümü (§6); `audit/runs.ts:63-74` | Depo-geneli kalem. Bu tool'a özgü not: bir `report` kartının isteyeceği üç alan (`pagesWithSchema`/`pageCount`, `typeCoverage`, `missingFields`) rapor nesnesinde hazır duruyor |  |

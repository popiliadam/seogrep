...
| B-7 | **P2** | **Tek satırlık zod mesajı ile ücret cümlesi arasında ayırıcı yok.** N4: `✖ Unrecognized key: "dataState" You were not charged.` — nokta ya da yeni satır yok. Çok satırlı mesajlarda ayırıcı VAR (N1, N5a-c: `→ at project_id\n\nYou were not charged.`). Dilim 2'de `audit_speed` B-6 olarak kaydedilen kusurun aynısı, **farklı bir tool ailesinde yeniden ölçüldü** — yani tool'a özgü değil, **yüzey geneli** | §3 N4; dilim 3'ün diğer üç tool'unda da birebir aynı (N11, N12, N13) | Registry'nin zod mesajı ile ücret cümlesini birleştirdiği tek yere ayırıcı girsin. Dilim 2'nin `audit_speed` B-6'sı ile **aynı düzeltmedir**; iki kayıt tek dilimde kapanmalı, ayrı ayrı değil |  |

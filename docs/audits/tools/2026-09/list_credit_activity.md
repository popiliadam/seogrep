...
**Karar (kapanış, 2026-09-02):** KAPANDI (dilim 1 düzeltmesi, #205 + #206 [CI'da, merge bekliyor]) — dört P1'in dördü de kapandı (B-1, B-2, B-3 canlı doğrulandı; B-4 hızlı şeritte pinlendi). **Kalan:** B-6 (P2, canlıda ölçülemedi — 0033 sonrası `project_id` taşıyan satır yok).
...
| B-4 | P1 | NEVER #4 kiracı filtresi hızlı şeritte ölçülmüyor: `.eq("user_id", userId)` silindiğinde 156 testin hepsi geçiyor. Koruma yalnız `*.db.test.ts`'te, o da `make verify`'da koşmuyor | M4 (§2) + `CLAUDE.md` kapı tablosu ("`make verify` … **DB şeritleri YOK**") | Kapının kapsam sorunu; bu tool'a özgü kod düzeltmesi değil. Öneri: `make verify-db`'nin hangi NEVER'ları tek başına taşıdığının zorlama haritasına yazılması | KAPANDI (#206, merge bekliyor) — üç okumanın da `.eq("user_id")`'i `service-client-pins.test.ts`'te pinli |

...
**Karar (kapanış, 2026-09-03):** **DÜZELTİLDİ — iki P1 de kapandı, üç P2 açık.** #217 (**CI'da**) B-1'i takvimi **veri olarak** getirerek kapattı (`gsc-data/google-updates.ts`, R-6.8/R-6.9 ile 17/17 birebir, doğrulama tarihi + 90 günlük bayatlık uyarısı; pencere bir güncellemeyle kesişince not listenin başında) ve B-2'yi gösterim + ortalama pozisyon değişimini satıra yazarak kapattı. **Kapanmayanlar adıyla:** B-3 ("Nothing left" satırının kendi cümlesi; caveat hâlâ footer'da), B-4 (13× pencere farkına sabit uygulanan eşikler) ve B-5'in eşik-açıklaması yarısı **AÇIK**. **Takvimin BAKIM yolu da AÇIK:** hiçbir kapı listeyi Google ile karşılaştırmıyor — ölçülen tek şey kendi doğrulama tarihinin yaşı, ve `goals/` hedefi yazılmadı. Canlı doğrulama **bekliyor**.

**Hakem eki (taze Opus, 2026-09-03 · 2/2 PASS).** Hakem bu kaydı PASS etti ve iki not düştü:
(1) **M3'ün eşdeğer-mutant iddiası bağımsız olarak doğrulandı** — aritmetik ayakta, bu satır bir test
deliği değil ve düzeltme kalemi olarak açılmamalı (§2); (2) B-5'in noktalama yarısı yeniden
çerçevelendi — kapanmış bir karar (#210) yanlışlıkla yeniden açılıyordu.

...
**Karar (kapanış, 2026-09-04):** **KISMEN DÜZELTİLDİ — üç P1'in ikisi tam kapandı (A-1 #222, A-2 #223 + canlı ✔), üçüncüsü (A-3) yarım: rezervasyon yarısı #222/#224'te kapandı, `item_types` enum'u ile hata metni operatörde.** A-8 ÇÜRÜTÜLDÜ (çağrı çıplak `target` ile yapılmıştı — bulgu kalkmadı, sebebi değişti) ve başarısızlık yolu yine de pinlendi. **Kalan:** A-4 AÇIK · A-5 KAPANDI #223 · A-6 canlıda hâlâ ölçülemedi · A-7 İMZA. Ölçüm turunun kararı SİLİNMEDİ (ders 16).
...
- Tutarsızlıklar: **yok** — karşılaştırılanlar: (1) canlı redlerin adlandırdığı sınırlar ↔ zod `:103-187`
  ...
  **SAYIM DÜZELTMESİ (hakem H-2, 2026-09-03; ders 11): "sıfır eşleşme" YANLIŞ — TEK eşleşme var.**
  `grep -niE "GSC|Search Console" my-pages.mdx` → `my-pages.mdx:92`, ve o satır tool'un kendi kapsamı
  hakkında değil, kredi paragrafının BAŞKA tool'ları sayan son cümlesidir (birebir):
  *"your existing credits are untouched and keep working for crawls, audits, reports and Search
  Console tools."* Yani sayfa `my_pages`'i Search Console'a hiçbir yerde bağlamıyor ve **§5'in
  sonucu değişmez** — değişen tek şey sayının kendisi. Ders 11: bir dizginin yokluğu iddia
  edilirken EN KISA AYIRT EDİCİ PARÇA ve `/i` ile aranır; buradaki ilk sayım tam da bunu kaçırdı.
...
**BÜTÇE OKUMASININ İKİ EKSENİ (hakem H-6, 2026-09-03; ders 7).** Yukarıdaki "2 ücretli çağrı, −80"
sayımı **MÜŞTERİ ekseninde** doğrudur: defterde üç `charge · my_pages` satırı var, ama N9'un
−40'ı 13 saniye sonra iade edildiği için müşteri gerçekten 80 kredi ödedi. **VENDOR ekseninde ise
istek sayısı ÜÇTÜR:** N9 satıcıya gitti, DFS bütçesinde bir rezervasyon açtı ve o rezervasyon
kapanmadı (A-3, şef gözlemi Ş-3 ile prod'da doğrulandı). İki eksen aynı sayıyla raporlanırsa
paylaşılan $3/gün tavanına düşen gerçek yük olduğundan az görünür. Bu kayıtta ikisi de adıyla durur:
**müşteri 2 ücretli / −80 kredi · vendor 3 istek / 1'i karşılıksız.**
...
**SAYIM DÜZELTMESİ (hakem H-2, 2026-09-03):** yukarıdaki "sıfır kez" §1'dekiyle aynı yanlış sayımdır —
`my-pages.mdx:92`'de bir eşleşme vardır ve o satır kredi paragrafının **başka tool'ları** sayan
cümlesidir ("…crawls, audits, reports and Search Console tools"). **§5'in sonucu değişmez:** sayfa
`my_pages`'in kendi kaynağını hiçbir yerde Search Console'a bağlamıyor; R-7.1/R-7.2/R-7.6/R-7.8
İLGİSİZ kararı ve aşağıdaki referans düzeltmesi aynen geçerli.
...
| # | şiddet | bulgu | kanıt | önerilen düzeltme (KOD YAZILMAZ, öneri) | durum (kapanış, 2026-09-04) |
|---|---|---|---|---|---|
| A-1 | **P1** | NEVER #4 — `crawl_pages` okumasının kiracı filtresi (`user_id`) silindiğinde **Docker'sız hiçbir kapı kırmızı vermiyor**. `service-client-pins.test.ts` bu zinciri hiç tanımıyor; `my-pages.test.ts`'in `userId` iddiası enjekte edilmiş bir double'a yapılıyor, gerçek sorguya değil | M7: `logs/m7.log` — `Test Files 155 passed (155)`, `Tests 4016 passed (4016)`. `grep -niE "my-pages\|my_pages\|crawl_pages" service-client-pins.test.ts` → sıfır eşleşme. `my-pages.test.ts:752-759` yalnız `LoadCrawlSideFn`'e geçen argümanı sınıyor | `service-client-pins.test.ts`'e `loadCrawlSide` zinciri için fake-query pini eklenmeli: `.eq("user_id", …)` çağrısının gerçekten YAPILDIĞI sınanmalı. Kapı `make verify`'ın koştuğu şeritte olmalı — bugünkü tek koruma `my-pages.db.test.ts (h)` ve o Docker istiyor. **Bu kayıt db şeridinin kırmızı verdiğini KANITLAMIYOR** (koşulmadı); hakem/CI o yarıyı ayrıca doğrulamalı | **KAPANDI #222** — `service-client-pins.test.ts`'e İKİ zincir pini: `crawl_pages` okuması ve `jobs`/`getLatestSucceededResult` portu (`whats_next` + üç audit'in ortak portu, hiç pinsizdi); `user_id`/`project_id`/`job_id`/`kind` silinince KIRMIZI. `my-pages.db.test.ts (d)/(h)` şeridi CI `verify-db`'de KOŞULDU ve yeşil geçti (şef CI'da okudu). **`goals/` hedefi hâlâ YOK** |
| A-2 | **P1** | Lokal uyarısı YOK. İki ücretli çağrının ikisi de en/2840 varsayılanında **1 satıcı sayfası** döndürdü (biri 100 sayfalık crawl'ı olan siteye karşı) ve çıktı sebebi hiç adlandırmıyor; verdiği tek öneri (`offset`'i ilerlet) `vendor_total_count == 1` iken işe yaramaz. Kardeş `ranked_keywords` aynı dersi kaynağında yazılı taşıyor, `my_pages` miras almamış | H1: adstark, `vendor_total_count` 1, crawl 5 sayfa. H2: dentnotion, `vendor_total_count` 1, crawl 100 sayfa. İkisi de `language en, location 2840`. Kaynak dersi: `tools/ranked-keywords.ts:145-152` ("the same 65 credits, twice, to discover a parameter the tool never mentioned") | `ranked_keywords`'ün `localeHint` + `twoLetterTld` çiftinin karşılığı buraya taşınmalı — **kopya değil, PAYLAŞILAN** bir yardımcı olarak (iki tool'un cümlesi ayrışırsa aynı domain iki farklı tavsiye alır). Eşik `my_pages` için satır sayısı değil `vendor_total_count` olmalı. `GENERIC_TWO_LETTER_TLDS` listesi ve "ülke kodunu TAHMİN ETME" kısıtı aynen korunmalı. **SINIF ATFI (hakem H-3, 2026-09-03):** bu bulgu **LOKAL-VARSAYILAN sınıfının** ölçülmüş ve en pahalı üyesidir (sınıf şiddeti **P1**, taşıyıcısı bu satır: 2 × 40 kredi zarar ÖLÇÜLDÜ). Öbür üyeler: `keyword_gap` G-3 (P2) · `ranked_keywords` (ağaçta `twoLetterTld`/`localeHint` taşıyan TEK tool — sınıfın azaltıcısının sahibi) · `discover_keywords` `for_site` (**ÖLÇÜLMEDİ**, mod hiç koşulmadı). Düzeltme tool başına değil SINIF olarak kesilmeli; bkz. `_DILIM4-HAKEM-SINIFLAR.md` sınıf 4 | **KAPANDI #223 + canlı ✔** (deploy `8cc06dd`, adstark 40 kredi) — `twoLetterTld` `ranked_keywords`'ten TAŞINDI (kopya değil), `format/locale-default.ts`. Canlı: *"This lookup used the DEFAULT locale — the United States, in English — but adstark.com.tr is a .tr domain, a two-letter country-code TLD …"*. Varsayılanlar DEĞİŞMEDİ |
| A-3 | **P1** | Şemanın reklam ettiği `item_types` değeri `"featured_snippet"` **beklenmedik sunucu hatası** üretiyor: kullanıcı `Tool "my_pages" failed unexpectedly … reference 457d2b7d` görüyor. Hata satıcı isteğinden SONRA oluşuyor: kredi defterinde charge+refund çifti var (net 0, doğru) ama DFS bütçe rezervasyonu **açık kalıyor** — yani paylaşılan $3/gün tavanından pay yiyor ve karşılığında hiçbir şey teslim edilmiyor | N9 canlı: `isError: true`, metin yukarıda birebir. Defter: `-40 charge my_pages` (19:31:13) → `+40 refund my_pages` (19:31:15). Enum kaynağı `dfs/relevant-pages.ts:258-263` (dört değer). Rezervasyon yolu `:706-709` + `:723-745`: `throw`'da `settleSpend`'e hiç ulaşılmıyor (dosyanın kendi yorumu bunu kasıtlı diye tarif ediyor). **KANIT — açık rezervasyon PROD'DA ölçüldü (şef gözlemi Ş-3, hakem turu 2026-09-03):** Supabase `public.dfs_spend`, spend_day 2026-09-03 → `relevant_pages/live · status=open · estimated 0.036 · actual null` (o günün 14 satırından tek `open` olanı); 19:31'de açıldı, ~20:15'te hâlâ `open`. Günlük tavana TAHMİNİYLE sayılıyor: `dfs_spend_today_usd` = 0,245920 bu satırı içeriyor. Yani "rezervasyon açık kalıyor" artık kod okumasından çıkan bir hipotez değil, prod okumasıdır. Kardeş vaka `discover_keywords` DK-3 (aynı sınıf, kendi yolu ölçülmedi) | Önce **TEŞHİS**: hata satıcıdan mı (non-20000 task) yoksa ayrıştırıcıdan mı geliyor — sunucu log'u `457d2b7d` referansıyla okunmalı; bu kayıt onu ölçemedi. Sonuca göre ya enum yalnız gerçekten çalışan değerlere daraltılmalı (şema reklam ettiğini teslim etmeli), ya da hata ücretsiz ve ANLAŞILIR bir redde çevrilmeli. Ayrıca: satıcı reddi kullanıcıya "unexpectedly" diye değil, satıcı reddi olduğu söylenerek dönmeli | **KISMEN — rezervasyon yarısı KAPANDI #222** (port hatasında `settleFailedSpend`; 0014 sayacı `coalesce(actual, estimated)` okuduğu için günlük toplam ve $3 kapısı sente kadar aynı — Fable hakem A hükmü, D'de yeniden doğrulandı; reaper `staleDfsReserves` artık yalnız gerçek çökmeleri sayar). **`item_types` enum'u ve "failed unexpectedly" metni İMZA KALEMİ — operatörde**; teşhis (`order_by organic.count` uyuşmazlığı) bir ÇIKARIMDIR — non-20000 task gözlenmedi |
| A-4 | P2 | Tekil/çoğul hataları canlıda iki kez bastı: `DataForSEO counts 1 pages matching this lookup in total` ve `Of the 1 page in this window whose address could be keyed, 1 also appear in the crawl…` | H1 ve H2 çıktıları (ikisinde de `vendor_total_count == 1`). Kaynak: `renderWindowCaption:388-390` (`whole` dalında çoğul yok) ve `renderMatchCount:653-655` (`also appear` sabit) | Her iki cümlede de `exactCount` sonucuna göre tekil/çoğul seçilmeli — dosya bunu zaten başka yerlerde doğru yapıyor (`rows === 1 ? "page" : "pages"`, `:382`). Saf fonksiyon, testi ucuz | **AÇIK — PR'da karşılığı bulunamadı** (`renderWindowCaption:388-390` ve `renderMatchCount:653-655` tekil/çoğul dalı dört diff'te yok; `format/quantities.ts` yalnız `MODEL_PRECISION_CLAUSE` taşıyor). Kardeşi `find_quick_wins` B-6 de açık |
| A-5 | P2 | `plan.mjs:127` `my_pages` EXCLUDED gerekçesi bayat: "Needs an operator budget signature." — 2026-09-02'nin "kredi sınırımız yok" kararından sonra geçersiz, ve bu turda tool iki kez ücretli koşuldu | Birebir satır §7'de. Defterde `charge · my_pages · project: adstark.com.tr` ve `… · project: dentnotion.com` | Ya EXCLUDED kaldırılıp PLAN satırlarına dönüştürülmeli (en az: varsayılan lokal + açık lokal çifti, A-2'yi kalıcı ölçmek için), ya da gerekçe bugünkü GERÇEK sebeple değiştirilmeli. Aynı bayat cümle `discover_keywords:126`'da da var — tek turda birlikte bakılmalı | **KAPANDI #223** (`plan.mjs:127` gerekçesi bugünkü gerçeğe çevrildi; kalan engel per-site locale, tool EXCLUDED kalıyor) |
| A-6 | P2 | Satıcı-yanı çıktı-tavanı yolu (`renderVendorLimitNote`, `vendorSideBudgets`, 18.000/9.000 bütçeleri) **canlıda hiç ölçülemedi**: iki özne de tek satır döndürdü. Yalnız birim testlerinde kanıtlı | H1 `limit: 25` → 1 satır; H2 `limit: 200` → 1 satır. Crawl-yanı tavanı ise ateşledi (50 basıldı / 49 basılmadı), yani mekanizmanın YARISI canlı doğrulandı | Ölçüm, satıcının çok satır döndürdüğü bir özneyle tekrarlanmalı — A-2 düzeltilirse doğru lokalde aynı siteler zaten çok satır döndürecek, yani tek çağrı iki bulguyu birden kapatabilir. **"Geçti" diye kapatılmamalı: bu kayıt onu ÖLÇMEDİ** | **AÇIK — canlıda ölçülemedi** — deploy `8cc06dd` turunda adstark YİNE tek satıcı satırı döndürdü, satıcı-yanı kesilme yolu ateşlenmedi. Bu turda da yalnız birim testlerinde kanıtlı |
| A-7 | P2 | Seçilebilirlik: "my pages" adı "sayfalarım Search Console'da nasıl" sorusunu çekiyor ama tool GSC'ye hiç bakmıyor. Azaltıcılar güçlü ve ölçüldü (description'ın ilk cümlesi satıcıyı adlandırıyor; `WHAT_THE_VENDOR_RETURNS` her cevapta basılıyor), kalan risk yalnız ADIN kendisinde | Description `:191-202`; `WHAT_THE_VENDOR_RETURNS` `:349` — canlıda iki cevapta da basıldı. Referans listesinin kendisi de bu tuzağa düşmüştü (§5) | Kod değil **imza kalemi**: ad değişikliği (örn. `ranking_pages`) müşteri yüzeyini kırar. Alternatif ve ucuz olan: docs sayfasına "Bu tool Search Console verisi OKUMAZ — o soru `pull_gsc_data`'nın" diye bir yönlendirme satırı. İnsan imzası gerekir | **İMZA KALEMİ — operatörde** (ad değişikliği müşteri yüzeyini kırar; ucuz alternatif docs'a tek yönlendirme satırı) |
| A-8 | P2 | **Başarısızlık yolunda defter satırı PROJE KAPSAMINI kaybediyor.** Aynı tool'un iki başarılı çağrısı deftere `project: adstark.com.tr` ve `project: dentnotion.com` kapsamıyla yazılırken, N9'un başarısız çağrısı hem charge hem refund satırını **`no project scope`** ile bıraktı. Dilim 3'ün H-1 ekseni (ücretin hangi projeye yazıldığı) bu üründe **yalnız mutlu yolda** ölçülmüştü; başarısızlık ekseni bugüne kadar hiç bakılmamış bir yüzeydi. Sonuç: bir tenant'ın başarısız ve iade edilmiş harcamaları defterde projesiz duruyor, yani proje bazlı bir harcama okuması onları göremez | Defter satırları (birebir, şef gözlemi Ş-2, 2026-09-03): `19:31:13 · -40 credits · charge · my_pages · no project scope` ve `19:31:15 · +40 credits · refund · my_pages · no project scope`; karşılaştırma H1/H2'nin `charge · my_pages · project: <domain>` satırları (§4). N9 çağrısı `project_id` ile YAPILMIŞTI — kapsam girdide vardı, deftere geçmedi | Teşhis önce: kapsamın nerede düştüğü ölçülmeli — `withCredits` meta'sı mı yoksa THROW→RELEASE yolundaki refund satırının kendi meta'sı mı. `handler-charge-scope-coverage.pin.test.ts` bugün yalnız çağrı yerini okuyor, **iade satırının kapsamını hiç sınamıyor** (bu turda ölçülmedi, iddia edilmiyor). Düzeltme A-3 ile birlikte tasarlanmalı: ikisi de aynı başarısızlık yolunun ürünü | **ÇÜRÜTÜLDÜ — ham kayıt #222'de okundu: başarısız çağrı ÇIPLAK `target` ile yapılmıştı, yani `no project scope` DOĞRU cevaptı.** Bulgu kalkmadı, sebebi değişti. Başarısızlık yolu yine de pinlendi (#222 `my-pages-failure-scope.pin.test.ts`: reserve `p_project_id` dolu, tam release, commit yok) |
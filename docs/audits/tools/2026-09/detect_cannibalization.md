...
**Karar (kapanış, 2026-09-03):** **KISMEN DÜZELTİLDİ — en zararlı vaka kapandı, SINIF kapanmadı.** #217 (**CI'da**) B-1'in **kök URL eksenini** kapattı: ana sayfa artık fold edilecek taraf olamaz, adıyla dışarıda bırakıldığı söyleniyor, fold listesi boşalırsa tavsiye hiç basılmıyor. **Sınıf KAPANMADI:** hakemin adlandırdığı hub/liste ekseni (`/doktorlarimiz/`) için hiçbir kapı yok — ölçüldü, `hub` / `listing page` / `category page` ne #217 diff'inde ne main'de geçiyor. **Kapanmayanlar adıyla:** B-2 (canonical bir direktif değil + aynı sayfanın zıt rolleri), B-3 (marka bastırma yalnız alan adı jetonunu tanıyor), B-4 (mutlak pozisyon tabanı yok) **AÇIK — üçü de iş emrine girmedi**; B-5'in yalnız noktalama yarısı kapandı. `goals/` hedefi ("birleştirme tavsiyesi kök URL'i fold edilecek taraf olarak adlandırmaz") **yazılmadı** — yani düzeltme bugün sessizce geri alınabilir; hakem bunu iki yönde de ölçmüştü. Canlı doğrulama **bekliyor**.

**Hakem eki (taze Opus, 2026-09-03 · 2/2 PASS).** Hakem bu kaydı PASS etti; **B-1 P1 olarak KALDI**
ve iki ölçümle güçlendi: (a) kök-URL dışlaması **eklenince de** 3914/3914 yeşil — eksen iki yönde de
pinsiz, düzeltme tek satır; (b) aynı canlı cümlede fold edilecek taraflardan biri bir **hub/liste
sayfası** (`/doktorlarimiz/`), yani yalnız `path === "/"` kapısı sınıfın **yarısını** açık bırakır.
Ayrıca B-5'in noktalama yarısı yeniden çerçevelendi (kapanmış bir karar yeniden açılıyordu).
...
- `goals/` hedefi gerekli mi: **EVET.** B-1 (kök URL koruması) bir kez eklenirse, onu geri alacak bir
  düzeltme hiçbir testi kırmaz — **hakem bunu ölçtü:** dışlamayı EKLEDİĞİNDE de 3914/3914 yeşil kaldı,
  yani hedef yazılmazsa düzeltme sessizce geri alınabilir — ve sonucu, bir müşteriye ana sayfasını canonical'lamasını

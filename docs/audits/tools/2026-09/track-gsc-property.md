...
|---|---|---|---|---|
| TGP-1 | P2 | Eşleme upsert'ünün çakışma hedefi (`onConflict: "user_id,project_id"`) HİÇBİR şeritte pinli değil. Hızlı şerit `mapProperty`'yi enjekte ediyor; db şeridi hedefi yalnız bir YORUMDA anıyor (`track-gsc-property.db.test.ts:282`), `expect` etmiyor. Kiracı kimliğini hedeften düşüren mutasyon 3680 testin hiçbirini kırmadı. | M2 (§2) | `defaultMapProperty`'yi db şeridinde HEAD-ON çağıran bir spec — `untrack-project.db.test.ts:204`'ün ("the archive WRITE itself is tenant-filtered") birebir muadili. Yorumdaki iddia bir `expect`e dönüşsün. |

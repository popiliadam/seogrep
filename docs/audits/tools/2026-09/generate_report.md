...
|---|---|---|
| 1 Statik | ÖLÇÜLDÜ | Handler `generate-report.ts` 229 satır; kredi `costs.ts:143` `generate_report: 15`; description ↔ docs ↔ canlı `tools/list` üçü de birebir aynı cümle; **vendor çağrısı YOK** (`dfs/`, `dataforseo`, `lighthouse` grep'i 0 eşleşme) |
...
**Karar (kapanış, YYYY-MM-DD):** düzeltme dalgası bittiğinde KAPATAN tur yazar (ders 16).

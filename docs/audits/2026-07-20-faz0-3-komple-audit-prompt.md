...
DENETİM BOYUTLARI (9/9 zorunlu; her boyutta en az listelenen kontroller — 1-8 UYGUNLUK denetimi,
9 GELİŞİM değerlendirmesidir ve raporun en az üçte birini hak eder):
...
9. ÜRÜN & GELİŞİM DEĞERLENDİRMESİ (uygunluk değil, İYİLEŞTİRME gözüyle — "neler eksik, neler
   olsa bu ürün daha iyi olur"). Yöntem: ürünü GERÇEKTEN KULLAN, sonra öner — masa başı fikir değil,
   kullanım gözlemi. Somut asgari koşular:
   a) DOGFOODING: test hesabıyla SeoGrep'in KENDİ araçlarını seogrep.com'a karşı koş (setup_project
      + crawl_site + audit_onpage/tech/schema + generate_report) — hem araç kalitesini yaşa hem
      kendi sitemizin gerçek SEO bulgularını rapora yaz (çifte değer).
   b) ARAÇ YÜZEYİ: 16 tool bir SEO ajansının/LLM agent'ının gerçek iş akışını ne kadar kapsıyor?
      Eksik tool/özellik adayları (ör. rakip analizi, backlink, rank tracking, içerik önerisi,
      çoklu-site yönetimi, zamanlanmış tarama, webhook/bildirim) — platinum-seo-engine'deki fikir
      seti FİKİR düzeyinde ilham kaynağıdır (AGPL: kod satırı asla). Tool ÇIKTILARININ LLM-client
      kalitesi: prose/structured dengesi, hata mesajlarının yönlendiriciliği, whats_next'in akıl
      yürütme kalitesi.
   c) WEBAPP/DASHBOARD UX: signup→key→client-kurulum→ilk-değer (time-to-value) sürtünmesi; eksik
      ekranlar/özellikler (kullanım grafiği, rapor yönetimi/silme, çoklu key, takım/işbirliği,
      bildirimler); bilinen boşluklar (landing'de Sign-in linki yok; gsc-banner; cap-mesajı
      redaksiyonu) + senin bulacakların.
   d) VİTRİN + DOCS + FUNNEL: landing→signup dönüşüm mantığı, pricing netliği, docs'un gerçek
      kurulum başarısı (5 client rehberini KENDİN izle — çalışıyor mu?), sitenin KENDİ SEO'su
      (dogfooding bulgularıyla birleştir).
   e) BETA/OPERASYON HAZIRLIĞI: rate-limit/abuse yüzeyi, izleme-alarm (uptime, 5xx, kuyruk
      birikimi, bütçe), log görünürlüğü, destek kanalı, status sayfası, yedekleme/felaket senaryosu.
   Her öneri: gözlem-kanıtı + ETKİ (yüksek/orta/düşük) × ÇABA (küçük/orta/büyük) + kova
   (QUICK-WIN / FAZ-4-STRATEJİK / ERTELENEBİLİR). Fiyat/kredi/paket önerileri tavsiye düzeyinde
   kalır (NEVER #6 — karar insanın).
...
## Yönetici özeti (≤12 satır: en kritik 3 bulgu + en değerli 3 gelişim önerisi + genel hüküm)
## Boyut raporları (1-8; her bulgu: kanıt + sınıf + öneri)
## Gelişim değerlendirmesi (boyut 9 — ayrı ve doyurucu bölüm):
   dogfooding sonuçları (kendi sitemizin SEO bulguları dahil) · öneri tablosu
   (öneri · gözlem-kanıtı · etki · çaba · kova) · önerilen FAZ 4 ADAY BACKLOG'u (sıralı)
## Açık borç envanteri (tablo: kayıt · kaynak · Faz-4-öncesi mi · öneri)
## FAZ 4 GO / NO-GO tavsiyesi (koşullu GO ise koşulları madde madde; go-kararına gelişim
   önerilerinden hangilerinin Faz-4-kapsamına alınmasını önerdiğini de ekle)

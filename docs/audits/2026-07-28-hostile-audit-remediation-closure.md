...
| H-06 | **PARTIAL** — teknik yarı FIXED ve artık **BAĞLI** | Vitrin private-beta, `/signup` açık; trial kilidi yalnız `auth.uid` | Aynı posta kutusundan 400 → **200**; yanlış-pozitif yönü 40 saldırgan çiftle sınandı; **çapraz-yol mutasyonu**: yalnız layout geri alınınca 5/8, yalnız callback geri alınınca 4/8 kırmızı; 7 kullanılamaz adres biçimi fail-OPEN | Fable PASS ×2 (0C/0I) | `0f7baae` `3e29431` `f6003bc` `a2378f9` · **wiring:** `1343929` `515f9c7` `ba0eb33` | **Politika yarısı KASTEN kapatılmadı** (operatör seçimi c). Kalan farm yüzeyi: 0020-öncesi trial'lar bir hak daha taşır · proton/Apple çoklu-alan namespace'leri ayrı fingerprint (gerçek posta kutusu başına ≤3) · sahip olunan/catch-all alan adı SINIRSIZ · disposable kasten yalnız sinyal. **Ters yön (reddetme) riski:** zaten trial almış biri YENİ bir adres doğrularsa o posta kutusunun fingerprint'i grant'siz kaydolur — o adres ileride başka gerçek bir kişiye geçerse reddedilir (nadir; kabul veya kuyruk). **Operasyonel ön koşul:** tasarım prod'da e-posta doğrulamanın AÇIK kalmasına dayanır. Gerçek kabul kapısı (CAPTCHA/`enable_signup`) Supabase panosunda |

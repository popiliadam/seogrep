...
kanıtı) → auth → tool yüzeyi 19/19 → ledger'dan bakiye okunuyor. Bu predicate PARASIZ ince dilimdir
...
## on-violation
Şüpheliler: registry değişikliği (19-tool pin kırıldıysa docs-schema-sync de bakar), auth yolu, credit_balances view, revoke edilmiş smoke key.
Runbook: tools/list sayısı ≠19 ise son merge'ün tool diff'ine bak → get_credit_balance hatasıysa fly logs + Supabase advisors → ledger tutarsızlığı şüphesinde İNSANI UYANDIR (contract.md: balance != SUM(ledger)). Otomatik düzeltme YOK.

...
kanıtı) → auth → tool yüzeyi 38/38 → ledger'dan bakiye okunuyor. Bu predicate PARASIZ ince dilimdir
...
[ "$(curl -sf --max-time 20 -X POST "$MCP_SMOKE_URL" -H 'content-type: application/json' -H 'accept: application/json, text/event-stream' -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | grep -o '"inputSchema"' | wc -l | tr -d ' ')" = "38" ]
...
## on-violation
Şüpheliler: registry değişikliği (38-tool pin kırıldıysa docs-schema-sync de bakar), auth yolu, credit_balances view, revoke edilmiş smoke key.
Runbook: tools/list sayısı ≠38 ise son merge'ün tool diff'ine bak → get_credit_balance hatasıysa fly logs + Supabase advisors → ledger tutarsızlığı şüphesinde İNSANI UYANDIR (contract.md: balance != SUM(ledger)). Otomatik düzeltme YOK.
...
### 36 → 38 — 2026-08-26, `list_jobs` + `list_credit_activity`

**BU BUMP BİR COMMIT GECİKMİŞ OLARAK YAZILIYOR, ve gecikmenin kendisi kayda değer.** Önceki beş
taşımanın hepsi registry değişikliğiyle AYNI commit'teydi; bu ikisi (imza md.15) deploy edildi ve
pin 36'da kaldı. Kimse fark etmedi çünkü bu kalem `MCP_SMOKE_URL` set değilken `exit 97` ile
**SKIP** ediyor, ve o oturumlarda kapı "16/16 PASS (5 skip)" diyordu — imzalı ders 7'nin tarif
ettiği şeyin birebir kendisi: env-koşullu SKIP, tam ölçüm gibi okundu.

Anahtar 2026-08-26'da yenilendiğinde kalem **ilk kez gerçekten koştu ve KIRMIZI verdi**. Ölçüldü:
canlı `inputSchema` sayısı **38**, predicate'in ikinci satırı (auth + canlı `get_credit_balance`)
**geçiyor** — yani ürün sağlam, bayat olan pin'di. Bu kalemin SKIP'i bu yüzden bir maliyet
kalemidir: iki gün boyunca üretim yüzeyini hiçbir şey saymadı.

### 35 → 36 — 2026-08-24, `serp_snapshot`

Beşinci taşıma, ve yine registry değişikliğiyle **AYNI COMMIT'te**. Sayı `costs.test.ts` (36) ve
`server.test.ts` (36) ile aynı anda oynadı; bu dilimde tek tool geldiği için pin bir arttı. İki
vakanın kuralı burada da geçerli: bu kalem merge ile MCP deploy'u ARASINDA kırmızıdır ve o
penceredeki yeşil, bu kalem için kanıt değildir. Bir önceki dilimin şerhi burada YOK: 0030
migration'ı üretimde zaten uygulanmıştır, yani deploy inince kalem doğrudan anlamlıdır.

# goal: landing-live
created: 2026-07-10
kaynak: Faz 1 İş A — canlı site markayı servis ediyor. Deploy öncesi SKIP (PROD_URL set değilken exit 97); ilk prod deploy insan kapısından geçince anlam kazanır.

## predicate
```predicate
[ -z "${PROD_URL:-}" ] && exit 97; curl -sf --max-time 15 "$PROD_URL" | grep -q SeoGrep
```

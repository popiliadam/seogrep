...
export const EXCLUDED = Object.freeze({
  ...
  // the reason that is still real: usually a PER-SITE INPUT the matrix does not carry, and for
  // the two backlink tools the price alone.
  //
  ...
  //
  // AND A THIRD TIME, on the axis the second pass got wrong. That pass wrote of audit_speed that
  // it "was not exercised paid in the 2026-09 round" — an UNMEASURED NEGATIVE, and false: the
  // Dilim 2 record has two charges on 2026-09-02 (2026-09-02T14:45:37Z and T14:47:40Z, -15 each)
  // and the #209 closure a third on 2026-09-03. A stale reason and a fabricated one fail the same
  // way, and the second is worse: it reads as a measurement. Every date below is a LEDGER date in
  // UTC, which is why the Dilim 5 runs say 2026-09-03 while their records are headed 2026-09-04 —
  // they were run late on the local evening (signed lesson 13: a claim nobody ran is a hypothesis).
  discover_keywords:
  ...
  link_gap:
    "paid, 45 credits/call. The budget signature arrived 2026-09-02 and the tool ran paid on " +
    "2026-09-03; the blocker is the same one keyword_gap has and is stated there in full — a " +
  ...
  backlink_changes:
    ...
    "2026-09-02 and the tool ran paid twice on 2026-09-03. NO per-site input is missing: a " +
  ...
  backlink_details:
    "paid, 35 credits/call against the DataForSEO backlinks API. The budget signature arrived " +
    "2026-09-02 and the tool ran paid twice on 2026-09-03, project-scoped and bare-target. Like " +
  ...
  disavow_candidates:
    ...
    "2026-09-03; the OTHER half of the old reason is still real and is why this stays excluded — " +
  ...
  audit_speed:
    "paid, 15 credits/call plus a Lighthouse run per URL. The tool ran paid in the 2026-09 round " +
    "too — twice on 2026-09-02 (Dilim 2 record, delta -30) and once on 2026-09-03 (closure, " +
    "delta -15); what still blocks a sweep cell is the per-site URL list: this tool audits named " +
    "URLs, and the matrix carries a domain.",
